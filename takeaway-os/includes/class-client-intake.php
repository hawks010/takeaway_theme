...
final class TTOS_Client_Intake {
    ...
    private static function request_detail(array $record): void {
        ...
        if (!empty($record['submitted_data'])) {
            ...
            echo '</ul><p class="ttos-muted">Menu uploads, brand guidelines, marketplace links, and policy free-text notes are stored for review and are not auto-applied blindly.</p></div>';
            echo '</div>';
        } else {
    ...
    private static function upload_specs(): array {
        return array(
            ...
            'menu_pdf'                 => array('extensions' => array('pdf'), 'max' => 8 * 1024 * 1024, 'multiple' => false),
            'menu_images'              => array('extensions' => array('jpg', 'jpeg', 'png', 'webp'), 'max' => 6 * 1024 * 1024, 'multiple' => true),
            'menu_spreadsheet'         => array('extensions' => array('csv', 'xls', 'xlsx'), 'max' => 6 * 1024 * 1024, 'multiple' => false),
        );
    }
...
<?php wp_footer(); ?>
    ...
    private static function import_summary_items(array $record): array {
        ...
        if (!empty($data['branding'])) {
            $items[] = 'Branding colours and logo/favicon attachment IDs where valid uploads exist';
        }
