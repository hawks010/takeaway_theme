...
final class TTOS_Setup_Health {
    ...
    private static function site_content_checks(): array {
        ...
        $placeholder_socials = array();
        foreach ($social_keys as $key) {
            $url = strtolower(trim((string) TTOS_Site_Content::get('social_links', $key, '')));
            if ($url !== '' && strpos($url, 'yourrestaurant') !== false) {
                $placeholder_socials[] = $key;
            }
        }
        $checks[] = self::make_check(
            'social_links_placeholder',
            'Social links are not placeholder demo URLs',
            empty($placeholder_socials),
            'No placeholder demo social URLs detected.',
            'Placeholder social URLs are still present for: ' . implode(', ', $placeholder_socials) . '. Remove or replace them in Site Content → Social Links.'
        );
    ...
    private static function shell_start(string $title, string $subtitle = ''): void {
        TTOS_Admin_Shell::render_start(array(
            'title' => $title,
            'subtitle' => $subtitle,
            'active' => 'takeaway-os-setup-health',
        ));
        self::notices();
    }
    ...
    private static function nav(): void {
        TTOS_Admin_Shell::render_primary_nav('takeaway-os-setup-health');
    }
