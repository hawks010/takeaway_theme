...
final class TTOS_Admin {
    ...
    public static function menu(): void {
        ...
        add_submenu_page('takeaway-os', 'Brand Guide', 'Brand Guide', 'ttos_manage_settings', 'takeaway-os-brand-guide', array(__CLASS__, 'page_brand_guide'));
        add_submenu_page('takeaway-os', 'Settings', 'Settings', 'ttos_manage_settings', 'takeaway-os-settings', array(__CLASS__, 'page_settings'));
    ...
    public static function assets(string $hook): void {
        ...
        wp_enqueue_media();
        wp_enqueue_style('ttos-admin', TTOS_URL . 'assets/admin.css', array(), TTOS_VERSION . '-admin-shell-5');
    ...
    public static function handle_posts(): void {
        ...
        if ($action === 'save_branding' && current_user_can('ttos_manage_settings')) {
            ...
            $choices = array(
                ...
                'header_style' => array('utility_header', 'centered_brand', 'solid', 'transparent'),
                'hero_style'   => array('editorial_split', 'cinematic_photo', 'product_mosaic', 'angled', 'minimal', 'photo'),
                'card_style'   => array('soft', 'outlined', 'flat'),
                'footer_style' => array('trust_led', 'editorial', 'dark', 'light'),
    ...
    public static function branding_form(string $return = '', string $anchor = '', string $button = 'Save branding'): void {
        $branding = TTOS_Settings::get('branding');
        ...
        self::select_field('Header style', 'branding[header_style]', $tokens['header_style'], array('utility_header' => 'Utility Header', 'centered_brand' => 'Centered Brand Header'));
        self::select_field('Hero style', 'branding[hero_style]', $tokens['hero_style'], array('editorial_split' => 'Editorial Split', 'cinematic_photo' => 'Cinematic Photo', 'product_mosaic' => 'Product Mosaic'));
        self::select_field('Card style', 'branding[card_style]', $tokens['card_style'], array('soft' => 'Soft shadow', 'outlined' => 'Outlined', 'flat' => 'Flat'));
        self::select_field('Footer style', 'branding[footer_style]', $tokens['footer_style'], array('trust_led' => 'Trust-Led Footer', 'editorial' => 'Editorial Footer'));
        echo '</div>';
        echo '<p class="ttos-muted">Editorial and cinematic hero layouts work best with a dedicated hero image. Fallback order is hero image, homepage/site content image, live menu image, then a neutral cuisine-aware illustration. Product mosaic builds itself from live menu items when no photo-led layout is wanted.</p>';
    ...
    public static function page_brand_guide(): void {
        self::shell_start('Brand Guide', 'One focused place for layout, colours, typography, and brand media. Changes here update the live site tokens immediately, so this becomes the post-launch home for visual refinements.');
        echo '<section class="ttos-card"><div class="ttos-card-head"><div><h2>How this fits the build</h2><p class="ttos-muted">The client onboarding wizard writes into the same design-token system this screen edits. Developers, owners, and managers keep one shared brand surface instead of a wizard-only dead end.</p></div><p><a class="ttos-mini" href="' . esc_url(admin_url('admin.php?page=takeaway-os-client-intake')) . '">Open client intake</a></p></div>';
        echo '<ul class="ttos-list">';
        echo '<li><span>Colours, modes, spacing and shadows</span><strong>Shared tokens</strong></li>';
        echo '<li><span>Header, hero and footer layout choices</span><strong>Theme-aware</strong></li>';
        echo '<li><span>Fonts, logo, favicon and hero media</span><strong>Live preview</strong></li>';
        echo '</ul></section>';
        self::branding_form('takeaway-os-brand-guide', 'branding', 'Save brand guide');
        self::shell_end();
    }
    ...
    public static function page_settings(): void {
        ...
        echo '<section class="ttos-card"><h2>Brand Guide</h2><p class="ttos-muted">Visual styling now lives on its own dedicated screen so owners and developers are not mixing design controls into runtime settings. Use Brand Guide for colours, fonts, logo, hero image, and layout style changes.</p><p><a class="ttos-button" href="' . esc_url(admin_url('admin.php?page=takeaway-os-brand-guide')) . '">Open Brand Guide</a></p></section>';
