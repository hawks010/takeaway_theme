...
1. Upload `takeaway-theme-v0.3.33-bundled.zip` in WordPress under `Appearance -> Themes -> Add New -> Upload Theme`.
...
### Update from older 1.3.x or 1.2.x builds
1. Upload/activate `takeaway-theme-v0.3.33-bundled.zip`.
2. `Appearance -> Takeaway Theme Setup` shows the bundled Takeaway OS `1.3.11` vs the installed version; click `Update bundled Takeaway OS`.
3. Settings, Site Content and orders are preserved (add-only migrations).
4. Run `Takeaway OS -> Setup Health` once after update and only use repair tools where the checks say they are needed.
5. Confirm `Takeaway OS -> Takeaway Tickets` is present for owner/manager/kitchen roles if the restaurant-role portal is enabled.
